...
class DirectionIndex:
    ...
    def __post_init__(self) -> None:
        ...
        if block.ndim != 2 or block.shape[0] != 3:
            raise ValueError(f"expected (3, measurements) directions, got shape {block.shape}")
        if block.shape[1] == 0:
    ...
    def nearest(self, direction: NDArray[np.floating]) -> DirectionWeights:
        ...
        distances = angular_distance_deg(unit_vectors(np.asarray(direction).reshape(3, 1)), self.directions)
        index = int(np.argmin(distances))
        return DirectionWeights(
            indices=np.array([index], dtype=np.int64),
            weights=np.array([1.0]),
            distance_deg=float(distances.ravel()[index]),
        )
    ...
    def nearest_k(self, direction: NDArray[np.floating], count: int = 3) -> DirectionWeights:
        ...
        query = unit_vectors(np.asarray(direction).reshape(3, 1))
        distances = angular_distance_deg(query, self.directions).ravel()
    ...
    def _triangulation(self) -> NDArray[np.int64] | None:
        ...
        try:
            ...
            hull = ConvexHull(self.directions.T)
        except Exception:
            return None
    ...
    def barycentric(self, direction: NDArray[np.floating]) -> DirectionWeights:
        ...
        query = unit_vectors(np.asarray(direction, dtype=np.float64).reshape(3, 1))[:, 0]
        ...
        for simplex in triangles:
            vertices = self.directions[:, simplex]  # (3, 3)
            try:
                weights = np.linalg.solve(vertices, query)
            except np.linalg.LinAlgError:  # pragma: no cover - degenerate triangle
                continue
            if np.all(weights >= -1e-9):
                distances = angular_distance_deg(query, vertices).ravel()
                return DirectionWeights(
