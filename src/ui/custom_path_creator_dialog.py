...
class CustomPathCreatorDialog(QDialog):
    ...
    def _load_profile(self) -> None:
        ...
        already_versioned = profile_schema_version(self.profile) >= PROFILE_VERSION
        self.store_metadata_check.setChecked(already_versioned)
        self.scene_units_spin.setValue(profile_transform(self.profile).scene_units_per_metre)
    ...
    def get_profile(self) -> dict:
        """The edited profile, with coordinate metadata only if it was asked for."""

        profile = self._raw_profile()
        if not self.store_metadata_check.isChecked():
            return profile
        ...
        profile.update(
            {
                ...
                "axisConvention": transform.axis_convention,
                "sceneUnitsPerMetre": float(transform.scene_units_per_metre),
                "centreScene": list(transform.centre_scene),
                "elevationM": float(transform.elevation_m),
            }
        )
        return profile
